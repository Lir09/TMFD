...
$("tbl-users").onclick = async (e) => {
  ...
  if (saveE) {
    const role = document.querySelector(`select[data-role="${saveE}"]`).value;
    const cls =
      document.querySelector(`select[data-class="${saveE}"]`).value || null;
    try {
...
$("tbl-classes").onclick = async (e) => {
  ...
  if (cid) {
    const name = document
      ...
      .value.trim();
    const grade =
      document.querySelector(`input[data-cgrade="${cid}"]`).value.trim() ||
      null;
    if (!name) {
